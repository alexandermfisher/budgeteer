...
public class MonzoAccountInformationProvider implements AccountInformationProvider {
    ...
    public MonzoAccountInformationProvider(MonzoProperties monzoProperties, RestClient monzoRestClient,
    ...
    public BankTokens exchangeCode(String code) {
        ...
        if (accessToken == null) {
            throw new ProviderException("No access token in Monzo response");
        }
    ...
    public BankTokens refreshTokens(String refreshToken) {
        ...
        String accessToken = (String) response.get("access_token");
        if (accessToken == null) {
            throw new ProviderException("No access token in refresh response");
        }
    ...
    public BankIdentity getIdentity(String accessToken) {
        ...
        try {
            ...
            if (response == null) {
                throw new ProviderException("Empty response from Monzo whoami endpoint");
            }

            String userId = response.userId();
            if (userId == null || userId.isBlank()) {
                throw new ProviderException("No user_id in Monzo whoami response");
            }
            ...
            throw new ProviderException("Failed to get Monzo user ID: " + e.getMessage(), e);
        }
    }
    ...
    public List<BankAccount> getAccounts(String accessToken) {
        ...
        try {
            ...
            throw new ProviderException("Failed to fetch Monzo accounts: " + e.getMessage(), e);
        }
    }
    ...
    public BankBalance getBalance(String accessToken, String accountId) {
        ...
        try {
            ...
            if (response == null) {
                throw new ProviderException("Empty response from Monzo balance endpoint");
            }
            return new BankBalance(response.balance(), response.currency());

        } catch (RestClientResponseException e) {
            handleMonzoError(e, "balance");
            throw new ProviderException("Failed to fetch Monzo balance: " + e.getMessage(), e);
        }
    }
    ...
    public BankTransactionPage getTransactions(
        ...
        try {
            ...
            if (root == null || !root.path("transactions").isArray()) {
                throw new ProviderException("Empty response from Monzo transactions endpoint");
            }
            ...
            throw new ProviderException("Failed to fetch Monzo transactions: " + e.getMessage(), e);
        }
    }
    ...
    private Map<String, Object> executeTokenRequest(MultiValueMap<String, String> formData) {
        try {
            ...
            if (response == null) {
                throw new ProviderException("Empty response from Monzo token endpoint");
            }
            ...
            throw new ProviderException("Failed to exchange/refresh tokens: " + e.getMessage(), e);
        }
    }
    ...
    private void handleMonzoError(RestClientResponseException e, String endpoint) {
        ...
        if (status.value() == 401) {
            ...
            throw new ProviderConnectionRevokedException(
        ...
        if (status.value() == 403) {
            ...
            if (body.contains("forbidden.verification_required")) {
                ...
                throw new ProviderReauthRequiredException(
            ...
            throw new ProviderException("Monzo API access denied. You may need to re-authorize.");
        }

        if (status.value() == 429) {
            log.warn("Monzo API rate limited for {}", endpoint);
            throw new ProviderException("Monzo API rate limit reached. Please try again later.");
        }
